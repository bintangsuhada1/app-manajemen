...
router.get('/options', requireAuth, allowRoles(...financeRoles), async (_, res) => {
...
router.get('/summary', requireAuth, allowRoles(...financeRoles), async (req, res) => {
  const rows = await prisma.transaction.findMany({ where: buildWhere(req.query) });
...
router.post('/', requireAuth, allowRoles(...writeRoles), async (req, res) => {
  ...
  res.status(201).json(await prisma.transaction.create({ data: serializePayload(parsed.data), include: { project: true } }));
});
